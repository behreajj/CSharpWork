...
public class ClrGradient : IEnumerable
{
    ...
    /// <param name="key">color key</param>
    /// <returns>this gradient</returns>
    ClrGradient InsortRight(in Key key)
    {
    ...
    {
        return ClrGradient.Eval(cg, step, (x, y, z) => Clr.MixRgbaLinear(x, y, z));
    }
        ...
        in Func<Clr, Clr, float, Clr> easing)
    {
        ...
        Key nextKey = keys[nextIdx];
        Key prevKey = keys[prevIdx];
